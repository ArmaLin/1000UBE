package com.dyaco.spirit_commercial.support.custom_view.wheelPicker;
...
public class WheelView<T> extends View implements Runnable {
    ...
    protected String getDataText(T item) {
        if (item == null) {
            ...
            if (mIntegerFormat.equals(MONTH_TEXT)) {
                return getTextMonth((Integer)item);
            } else {
                //如果为整形则最少保留两位数.
...
//                    : String.valueOf(item);
    ...
    private String getTextMonth(int month) {

        String[] shortMonths = new DateFormatSymbols(Locale.getDefault()).getMonths();
        String monthText;

        monthText = shortMonths[month - 1];
