...
class SpiritTextView2 @JvmOverloads constructor(
    context: Context,
    ...
    // ✅ 把每段 path 分開來（來自你提供的 SVG，每段都是一個字）
    private val svgPathList = listOf(
        ...
        "M349.136 0C347.144 0 345.65 1.24514 345.152 2.98833L343.16 8.96498H373.292L356.856 63.751H375.782C377.774 63.751 379.268 62.5059 379.766 60.7627L395.206 8.96498H417.619C419.362 8.96498 420.856 8.46693 421.852 7.47082L430.568 0H349.136Z",
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        val originalWidth = 430.568f
        val originalHeight = 64f

        val scale = min(w / originalWidth, h / originalHeight)
        val dx = (w - originalWidth * scale) / 2f
        val dy = (h - originalHeight * scale) / 2f
        ...
        scaledPath.reset()
        combinedPath.transform(matrix, scaledPath)
        ...
        animator = ValueAnimator.ofFloat(0f, 1f).apply {
            ...
            addUpdateListener {
                this@SpiritTextView2.animatedFraction = it.animatedValue as Float
                invalidate()
            }
            start()
        }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        animatedPath.reset()
        pathMeasure?.let { measure ->
            val stop = animatedFraction * measure.length
            measure.getSegment(0f, stop, animatedPath, true)
        }
