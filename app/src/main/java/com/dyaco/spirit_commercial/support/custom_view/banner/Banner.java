package com.dyaco.spirit_commercial.support.custom_view.banner;
...
import com.dyaco.spirit_commercial.support.custom_view.banner.config.BannerConfig;
import com.dyaco.spirit_commercial.support.custom_view.banner.config.IndicatorConfig;
import com.dyaco.spirit_commercial.support.custom_view.banner.indicator.Indicator;
import com.dyaco.spirit_commercial.support.custom_view.banner.listener.OnBannerListener;
import com.dyaco.spirit_commercial.support.custom_view.banner.listener.OnPageChangeListener;
import com.dyaco.spirit_commercial.support.custom_view.banner.transformer.ScaleInTransformer;
import com.dyaco.spirit_commercial.support.custom_view.banner.util.BannerLifecycleObserver;
import com.dyaco.spirit_commercial.support.custom_view.banner.util.BannerLifecycleObserverAdapter;
...
import com.dyaco.spirit_commercial.support.custom_view.banner.util.ScrollSpeedManger;
...
public class Banner<T, BA extends BannerAdapter> extends FrameLayout implements BannerLifecycleObserver {
    private boolean mBounceEffectEnabled = false;
    ...
    private void init(Context context) {
        ...
        // 在這裡呼叫 enableBounceEffect() 啟用彈力效果
...
//        }
    ...
    public BA getAdapter() {
        if (mAdapter == null) {
            LogUtils.e(getContext().getString(R.string.error_code_log));
        }
        return mAdapter;
    }
    ...
    public Banner setAdapter(BA adapter) {
        if (adapter == null) {
            throw new NullPointerException(getContext().getString(R.string.error_code_log));
        }
    ...
    public Banner setBounceEffectEnabled(boolean enabled) {
        mBounceEffectEnabled = enabled;
        //    RecyclerView recyclerView = (RecyclerView) mViewPager2.getChildAt(0);
        if (enabled) {
            // 啟用自訂的彈力效果
            enableBounceEffect();
//        } else {
//            // 恢復 RecyclerView 的預設 EdgeEffectFactory，使用系統原生的彈力效果
...
//            });
    ...
    private void enableBounceEffect() {
        RecyclerView recyclerView = (RecyclerView) mViewPager2.getChildAt(0);
        ...
        recyclerView.setEdgeEffectFactory(new RecyclerView.EdgeEffectFactory() {
            ...
            protected EdgeEffect createEdgeEffect(@NonNull RecyclerView view, int direction) {
                // 根據方向決定 multiplier：
                // DIRECTION_LEFT 表示左側邊界，pull 時向右偏移 (正值)
                // DIRECTION_RIGHT 表示右側邊界，pull 時向左偏移 (負值)
                ...
                return new EdgeEffect(view.getContext()) {
                    private final float OVERSCROLL_FACTOR = 0.3f;
                    private SpringAnimation springAnim;

                    @Override
                    public void onPull(float deltaDistance, float displacement) {
                        // 根據 multiplier 調整偏移方向
                        float pullTranslation = view.getTranslationX()
                                + multiplier * view.getWidth() * deltaDistance * OVERSCROLL_FACTOR;
                        view.setTranslationX(pullTranslation);
                    }

                    @Override
                    public void onRelease() {
                        if (springAnim == null) {
                            springAnim = new SpringAnimation(view, DynamicAnimation.TRANSLATION_X, 0f);
