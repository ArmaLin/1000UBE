 ...
 */
class SpiritTextView @JvmOverloads constructor(
    context: Context,
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        // 總原始寬度 (各圖固有尺寸的寬度相加)
        val totalOriginalWidth = svgWidth1 + svgWidth2 + svgWidth3 + svgWidth4 + svgWidth5 + svgWidth6  // 105+96+42+96+42+88 = 469
        // 總間距：前 4 組圖採用 defaultImageSpacing，最後一組圖需加上額外的間距
        val totalSpacing = defaultImageSpacing * 4 + (defaultImageSpacing + extraSpacingBetween5And6)
        // 整組圖形的總寬度
        val totalGroupWidth = totalOriginalWidth + totalSpacing
        // 圖形本來的高度（SVG 定義）
        val groupHeight = svgHeight

        // 計算位移，將整組圖形置中
        val dxTotal = (w - totalGroupWidth) / 2f
        val dy = (h - groupHeight) / 2f

        // 計算每個圖形的 x 軸起始位移
        // 注意：這裡依序累加前一圖的寬度與間距（此處不做縮放處理）
        val x1 = dxTotal
        val x2 = x1 + svgWidth1 + defaultImageSpacing
        val x3 = x2 + svgWidth2 + defaultImageSpacing
        val x4 = x3 + svgWidth3 + defaultImageSpacing
        val x5 = x4 + svgWidth4 + defaultImageSpacing
        val x6 = x5 + svgWidth5 + (defaultImageSpacing + extraSpacingBetween5And6)
        ...
        // 將原始 SVG 路徑依據平移矩陣轉換到對應的 scaledPath (因為不縮放，所以保持原始大小)
        ...
        // 取得所有圖形的路徑長度，並選出最大者作為動畫歸一化進度的依據
        ...
        animator = ValueAnimator.ofFloat(0f, 1f).apply {
            ...
            addUpdateListener { animation ->
                this@SpiritTextView.animatedFraction = animation.animatedValue as Float
                invalidate()
            }
            start()
        }
