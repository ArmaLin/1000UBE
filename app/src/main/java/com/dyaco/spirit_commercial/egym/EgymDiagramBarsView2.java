...
public class EgymDiagramBarsView2 extends View {
    ...
    private EgymDiagramBarsView2.Bar[] mBars; //incline

    private Paint mBarPaint2;
    private EgymDiagramBarsView2.Bar[] mBars2; //level

    private Paint mBarPaint3;
    private EgymDiagramBarsView2.Bar[] mBars3; //blank
    ...
    private EgymDiagramBarsView2.LevelChangedListener mLevelChangedListener;
    private EgymDiagramBarsView2.LevelShowMsgListener levelShowMsgListener;
    ...
    public EgymDiagramBarsView2(Context context) {
        this(context, null);
    }

    public EgymDiagramBarsView2(Context context, AttributeSet attributeSet) {
    ...
    public EgymDiagramBarsView2.Bar[] getBars() {
        return this.mBars;
    }

    public void setLevelChangedListener(EgymDiagramBarsView2.LevelChangedListener listener) {
    ...
    private void initBars() {
        mBars = new EgymDiagramBarsView2.Bar[mBarCount];
        mBars2 = new EgymDiagramBarsView2.Bar[mBarCount];
        mBars3 = new EgymDiagramBarsView2.Bar[mBarCount];

        mBarSpaces = new float[mBarCount]; // 初始化間距陣列

        for (int i = 0; i < mBars.length; ++i) {
            mBars[i] = new EgymDiagramBarsView2.Bar();
            mBars[i].setLevel(0);
            mBars[i].setColor(ContextCompat.getColor(context, R.color.colorCd5bff));

            mBars2[i] = new EgymDiagramBarsView2.Bar();
            mBars2[i].setLevel(0);
            mBars2[i].setColor(ContextCompat.getColor(context, R.color.color1396ef));

            mBars3[i] = new EgymDiagramBarsView2.Bar();
            mBars3[i].setLevel(0);
   ...
   //     drawBars(canvas);
    ...
    private void drawBars(Canvas canvas, EgymDiagramBarsView2.Bar[] bars, Paint paint, int count, int barType) {
    ...
    public void setLevelShowMsgListener(EgymDiagramBarsView2.LevelShowMsgListener listener) {
...
//            int popupX = (int) (barsViewLocation[0] + whiteLineX - textViewWidth / 2);
    ...
    public EgymDiagramBarsView2.Bar getBar(int barType, int barPosition) {
        EgymDiagramBarsView2.Bar[] bars;
        if (barType == BAR_TYPE_INCLINE) { // incline
            bars = mBars;
        } else if (barType == BAR_TYPE_SPEED) { // level
    ...
    // 根據 mBarSpaces > 0 的位置劃分區段
    private List<Segment> computeSegments() {
        List<Segment> segments = new ArrayList<>();
        int segStart = 0;
        for (int i = 0; i < mBarCount - 1; i++) {
            if (mBarSpaces[i] > 0) {
                segments.add(new Segment(segStart, i));
                segStart = i + 1;
            }
        }
        if (segStart < mBarCount) {
            segments.add(new Segment(segStart, mBarCount - 1));
        }
        return segments;
    }

    // 根據劃分的區段決定逐一或合併繪製
    private void drawBarsMerged(Canvas canvas) {
        List<Segment> segments = computeSegments();
        for (int segIndex = 0; segIndex < segments.size(); segIndex++) {
            ...
            if (segIndex == 0) {
                ...
                // 其他區段合併繪製
                drawMergedBar(canvas, seg, BAR_TYPE_SPEED);
                drawMergedBar(canvas, seg, BAR_TYPE_INCLINE);
                if (isShowBar3) {
                    drawMergedBar(canvas, seg, BAR_TYPE_BLANK);
                }
            }
            // 每個區段結束後，依據 mBarSpaces 設定繪製 spacing
            if (seg.end < mBarCount - 1 && mBarSpaces[seg.end] > 0) {
                float gapLeft = getBarLeft(seg.end + 1) - mBarSpaces[seg.end];
                float gapRight = getBarLeft(seg.end + 1);
                Paint clearPaint = new Paint();
                clearPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
                canvas.drawRect(gapLeft, 0, gapRight, mViewHeight, clearPaint);
            }
        }
    }


    // 合併繪製區段內的 bar，內部以透明區塊模擬各 bar 之間的 spacing
    private void drawMergedBar(Canvas canvas, Segment seg, int barType) {
        float left = getBarLeft(seg.start);
        float right = getBarLeft(seg.end) + mBarWidth;
        Bar[] bars;
        Paint paint;
        if (barType == BAR_TYPE_INCLINE) {
            bars = mBars;
        ...
        float level = bars[seg.start].getLevel();
        float negativeLevel = mBarMaxLevel - level;
        float top = mLevelUnitY * negativeLevel;
        if (MODE == ModeEnum.CT1000ENT && barType == BAR_TYPE_INCLINE && UNIT_E == DeviceIntDef.METRIC) {
            top = ((float) mViewHeight / 15) * (15 - level);
        }
        ...
        if (barType == BAR_TYPE_INCLINE) {
            ...
            float barHeightS = mViewHeight - (mLevelUnitY * (mBarMaxLevel - mBars2[seg.start].getLevel()));
            if (isShowBar3 && barHeightI <= barHeightS) {
                int bColor1 = ContextCompat.getColor(context, R.color.color8673ED);
                int bColor2 = ContextCompat.getColor(context, R.color.color3E357A);
                iColor1 = bColor1;
                iColor2 = bColor2;
            }
            gradientColors = new int[]{iColor1, iColor2};
        } else if (barType == BAR_TYPE_SPEED) {
            int sColor1 = ContextCompat.getColor(context, R.color.color1396ef);
            int sColor2 = Color.parseColor("#083C64");
            gradientColors = new int[]{sColor1, sColor2};
        } else {
            int bColor1 = ContextCompat.getColor(context, R.color.color8673ED);
            int bColor2 = ContextCompat.getColor(context, R.color.color3E357A);
            gradientColors = new int[]{bColor1, bColor2};
        }
        LinearGradient dynamicGradient = new LinearGradient(0, top, 0, bottom, gradientColors, null, Shader.TileMode.CLAMP);
        paint.setShader(dynamicGradient);
        paint.setXfermode(null);
        canvas.drawRect(left, top, right, bottom, paint);

        // 模擬區段內每根 bar 之間的 spacing
        for (int i = seg.start + 1; i <= seg.end; i++) {
            if (i - 1 < mBarSpaces.length && mBarSpaces[i - 1] > 0) {
                float gapLeft = getBarLeft(i) - mBarSpaces[i - 1];
                float gapRight = getBarLeft(i);
                Paint clearPaint = new Paint();
                clearPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
                canvas.drawRect(gapLeft, top, gapRight, bottom, clearPaint);
            }
        }
    }

    private float getBarLeft(int index) {
        float left = mSideWidth;
